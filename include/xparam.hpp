...
public:
     ...
     * pointer of dom-root node to the parameter.
     */
    void loadXmlDoc(const string &xdoc, XmlParser *parser = NULL, bool checksum = false,
                    const string &iv = "");
    /**
    ...
    void saveXmlDoc(const string &xdoc, bool show_runtime = false, const int &indent = 0,
                    bool with_endl = false, bool checksum = false, const string &iv = "") const;
    /**
     ...
     */
    string xml(bool show_runtime = false, const int &indent = 0, bool with_endl = false) const;
    /**
     * Print out parameter value in xml format.
     * \param indent size of indention.
     * \param endl 	 string would be used as end-line character.
     */
    virtual string _xml(bool show_runtime, const int &indent, const string &endl) const = 0;
    /**
...
protected:
     ...
     */
    bool dont_show(bool show_runtime) const { return is_runtime() && !show_runtime; }
 ...
 * of <XParam *>.
 */
template <typename List = std::vector<XParam *>> class _XMixParam : public XParam
{
public:
     ...
     * \param [in] parentNode Parent XParam of this object
     */
    virtual void dbCreateStructure(const XParam *parentNode = (XParam *)NULL);
    /**
     * Destroys related databse's data structure
     * \param [in] parentNode Parent XParam of this object
     */
    virtual void dbDestroyStructure(const XParam *parentNode = (XParam *)NULL);
    ...
    virtual string generateJoinStmts(const XParam *parentNode = (XParam *)NULL);
 ...
 */
template <typename T> class XIntParam : public XSingleParam
{
public:
    ...
    XIntParam(const _XIntParam &iparam) :
        XSingleParam(iparam.pname), min(iparam.min), max(iparam.max), val(iparam.val)
    {
    }
    XIntParam(XIntParam &&_xip) :
        XSingleParam(std::move(_xip)), min(_xip.min), max(_xip.max), val(_xip.val)
    {
    }
...
protected:
    bool checkLimit() { return max >= min; }
...
public:
    ...
    void set_value(const XFloat &value) { (*this) = value; }
 ...
 */
template <typename T> class XEnumParam : public XSingleParam
{
public:
    /**
     ...
     */
    XEnumParam(const string &_pname, unsigned short _def) :
        XSingleParam(_pname), def(_def), val(_def)
    {
    }
    XEnumParam() : XEnumParam("value", T::MAX) {}
    XEnumParam(XEnumParam &&_xep) : XSingleParam(std::move(_xep)), def(_xep.def), val(_xep.def) {}
    ...
    virtual XParam &operator=(const string &str)
    {
        ...
        throw Exception("Bad '" + pname + "' value !", TracePoint("pparam"));
    }
    ...
    {
        if (value >= 0 && value <= T::MAX)
            val = value;
        else
            throw Exception("Bad <" + pname + "> value !", TracePoint("pparam"));
    }
    virtual int get_value() const { return val; }
 ...
 */
template <typename T, typename Key = int, typename List = std::vector<XParam *>>
...
public:
    ...
    XSetParam(const string &_pname) : XMixParam(_pname), smapEnabled(false) {}
    XSetParam(XSetParam &&_xsp) :
        XMixParam(std::move(_xsp)), smap(std::move(_xsp.smap)), smapEnabled(_xsp.smapEnabled)
    {
    ...
    virtual void addParam(XParam *param)
    {
        ...
        if (sparam == NULL) {
            throw Exception("Bad T param in addParam", TracePoint("pparam"));
        }
     ...
     */
    template <class Pred> T *query(Pred pred)
    {
        for (iterator iter = begin(); iter != end(); ++iter) {
            if (pred((T *)*iter))
                return ((T *)*iter);
     ...
     */
    virtual void del(iterator iter) { delete del_soft(iter); }
    /**
     ...
     */
    template <class Compare> void sort(Compare compare, SortMode mode = SortMode::ASCENDING)
    {
     ...
     */
    template <class CallBack> void iterate(CallBack callback)
    {
        for (iterator iter = begin(); iter != end(); ++iter) {
            callback((T *)*iter);
        }
     ...
     */
    template <class CallBack> bool iterate_until(CallBack callback)
    {
        for (iterator iter = begin(); iter != end(); ++iter) {
            if (!callback((T *)*iter))
                return false;
...
protected:
    ...
    void add2SMap(iterator iter)
    {
        ...
        /* we can't store parameter with null id. */
        if (!tparam->key(_key)) {
            throw Exception("Parameter doesn't have any key !", TracePoint("pparam"));
        }
        /* we can't store parameters with same IDs. */
        if (keyExist(_key)) {
            throw Exception("Duplicated key parameter !", TracePoint("pparam"));
        }
        _add2SMap(_key, iter);
    }
    /**
     ...
     */
    virtual bool keyExist(const Key &_key) { return smap.find(_key) != smap.end(); }
    /**
     * Put proper elmenet at proper place in smap.
     */
    virtual void _add2SMap(const Key &_key, const iterator &iter) { smap[_key] = *iter; }
    /**
     * Clear content of smap.
     */
    virtual void clearSMap() { smap.clear(); }
    /**
    ...
    virtual T *newT(const XmlNode *node)
    {
        T *t = new T;
        if (t == NULL)
            throw Exception("Can't allocate memory !", TracePoint("pparam"));
        return t;
    }
    virtual T *newT(const T &t) { return newT((const XmlNode *)NULL); }
};
...
public:
    ...
    XListParam(XListParam &&_xlp) : _XISetParam(std::move(_xlp)), smap(std::move(_xlp.smap)) {}
 ...
 * \endcode
 */
template <typename T> class XEnumSetParam : public XSetParam<XEnumParam<T>>
{
public:
     ...
     */
    XEnumSetParam(const std::string &_pname, const std::string &_tagName = "value") :
