...
template <typename T> class XListNode;

template <typename T, typename _Alloc = std::allocator<XListNode<T>>> class XList;

template <typename T> class XListIterator;
template <typename T> class XListConstIterator;
...
public:
     ...
     * \see inc_usage
     */
    void access() { inc_usage(); }
    /**
     * End of access to the node.
     * Decrement usageNO by "1".
     * \see dec_usage
     */
    void release() { dec_usage(); }
    int get_usage() { return usageNO; }
    void lock() { pthread_mutex_lock(&n_lock); }
    void unlock() { pthread_mutex_unlock(&n_lock); }
    /**
...
protected:
     ...
     * at deletion time.
     */
    void inc_usage(unsigned int _inc = 1) { usageNO += _inc; }
    /**
    ...
    void dec_usage(unsigned int _dec = 1)
    {
        usageNO -= _dec;
        if (usageNO <= 0)
            pthread_cond_signal(&usage_cond);
    }
    /**
 ...
 */
template <typename T> class XListNode : public _XListNode_base
{
...
public:
    ...
    XListNode(const T &_data) : _XListNode_base(), _M_data(_data) {}

    T &get_data() { return _M_data; }
...
public:
    ...
    explicit XListIterator(_XNodeBase *_list_end, _XNodeBase *__x)
    {
        lockedNode = NULL;
        _M_node = __x;
        list_end = _list_end;
    ...
    reference operator*() const { return static_cast<_XNode *>(_M_node)->_M_data; }
    pointer operator->() const { return &(static_cast<_XNode *>(_M_node)->_M_data); }
    _Self &operator++()
    {
        lock();
        release();
    ...
    bool operator==(const _Self &rvalue) const { return _M_node == rvalue._M_node; }
    bool operator!=(const _Self &rvalue) const { return _M_node != rvalue._M_node; }
    /**
     ...
     */
    bool is_finied() const { return (this->_M_node == NULL); }
    /**
     * Is this iterator finished? reached at end.
     */
    bool is_end() const { return (this->_M_node == list_end); }
    /**
     * Do iterates on any node?
     */
    bool on_element() const { return ((this->_M_node != list_end) && (this->_M_node != NULL)); }
    bool on_node() const { return ((this->_M_node != list_end) && (this->_M_node != NULL)); }
    /**
     * Reset the iterator.
...
public:
    ...
    explicit XListConstIterator(const _XNodeBase *_list_end, const _XNodeBase *__x)
    {
        lockedNode = NULL;
        _M_node = __x;
        list_end = _list_end;
    ...
    reference operator*() const { return static_cast<const _XNode *>(_M_node)->_M_data; }
    pointer operator->() const { return &(static_cast<const _XNode *>(_M_node)->_M_data); }
    _Self &operator++()
    {
        lock();
        release();
    ...
    bool operator==(const _Self &rvalue) { return _M_node == rvalue._M_node; }
    bool operator==(const _XListIterator &rvalue) { return *this == _Self(rvalue); }
    bool operator!=(const _Self &rvalue) const { return _M_node != rvalue._M_node; }
    bool operator!=(const _XListIterator &rvalue) const { return *this != _Self(rvalue); }
    /**
     * Finalize the iteration.
     *
...
template <typename T, typename _Alloc> class _XList_base
{
protected:
    typedef typename _Alloc::template rebind<XListNode<T>>::other _Node_alloc_type;
    ...
    struct _XList_impl : public _Node_alloc_type {
        ...
        _XList_impl() : _Node_alloc_type(), _M_node() {}

        _XList_impl(const _Node_alloc_type &__a) : _Node_alloc_type(__a), _M_node() {}

#ifdef __GXX_EXPERIMENTAL_CXX0X__
        _XList_impl(_Node_alloc_type &&__a) : _Node_alloc_type(std::move(__a)), _M_node() {}
#endif

        void lock() { _M_node.lock(); }
        void unlock() { _M_node.unlock(); }
    };

    _XList_impl _M_impl;

    XListNode<T> *_M_get_node() { return _M_impl._Node_alloc_type::allocate(1); }
    void _M_put_node(XListNode<T> *__p) { _M_impl._Node_alloc_type::deallocate(__p, 1); }
...
public:
    ...
    _T_alloc_type _M_get_T_allocator() const { return _T_alloc_type(_M_get_Node_allocator()); }
 ...
 * accesses finished.
 */
template <typename T, typename _Alloc> class XList : public _XList_base<T, _Alloc>
{
public:
...
protected:
    ...
    _XNode *_M_create_node(const value_type &__x)
    {
        ...
        __try {
            _M_get_T_allocator().construct(std::__addressof(__p->_M_data), __x);
        }
        __catch(...)
        {
            _M_put_node(__p);
     ...
     *  @a __args in it.
     */
    template <typename... _Args> _XNode *_M_create_node(_Args &&...__args)
    {
        _XNode *__p = this->_M_get_node();
        __try {
...
public:
    ...
    {
        for (const_iterator iter = xlist.begin(); iter != xlist.end(); ++iter) {
            push_back(*iter);
        }
    }
    iterator begin()
    {
        this->_M_impl.lock();
        ;
        iterator iter(&this->_M_impl._M_node, this->_M_impl._M_node._M_next);
        this->_M_impl.unlock();
        ;
        return iter;
    }
    const_iterator begin() const
    {
        ...
        const_iterator iter(&this->_M_impl._M_node, this->_M_impl._M_node._M_next);
        ((_XList *)this)->_M_impl.unlock();
        return iter;
    }
    iterator end() { return iterator(&this->_M_impl._M_node, &this->_M_impl._M_node); }
    const_iterator end() const
    {
        return const_iterator(&this->_M_impl._M_node, &this->_M_impl._M_node);
    }
