...
public:
    ...
    static StatusTransition canTransit(int from, int to) { return transitionTable[from][to]; }
};

template <typename Type> class XObject;
template <typename Type> class XObjectList;
 ...
 */
template <typename Type> class XObjectConnection
{
public:
    ...
    XObjectConnection(const string &_name, const string &_role, bool _notify, ConnectionType _type,
                      iterator _oside) :
        cid("cid")
    {
        name = _name;
    ...
    XObjectConnection(const string &_name = "", const string &_role = "", bool _notify = false,
                      ConnectionType _type = WEAK) :
        cid("cid")
    {
        name = _name;
        role = _role;
    ...
    XObjectConnection(const XObjectConnection &c) : cid("cid") { *this = c; }
     ...
     */
    void regenerateCID() { cid.regenerate(); }
    /**
    ...
    string xml() const
    {
        ...
        ret += "<notify>" + string((notify) ? "true" : "false") + "</notify>";
        ret += "<type>" + string((type == WEAK) ? "weak" : "strong") + "</type>";
        ret += "<oside>" + (*oside)->get_key() + "</oside>";
        ret += "<oside_name>" + ((_XObject *)(*oside))->get_name() + "</oside_name>";
        ret += "</xobj_connection>";
        return ret;
    }
    string shell_xml() const
    {
        ...
        xml += "<column>" + string(notify ? "notify" : "no notify") + "</column>";
        xml += "<column>" + string(type == WEAK ? "weak" : "strong") + "</column>";
        xml += "<column>" + (*oside)->get_key() + "</column>";
        xml += "<column>" + ((_XObject *)(*oside))->get_name() + "</column>";
 ...
 */
template <typename Type> class XObjectScanner
{
public:
    typedef XObject<Type> _XObject;
    ...
    XObjectScanner() { use_name = use_role = use_notify = use_type = use_oside = use_cid = false; }
    XObjectScanner(const _XObjectConnection &c)
    {
        use_name = use_role = use_notify = use_type = use_oside = use_cid = false;
        cond = c;
    }
    /**
     * Scan "xobj".
     ...
     */
    virtual ScannerAction scan(_XObject *xobj) { return SA_END; }
    /**
    ...
    {
        if (use_name && (c.name != cond.name))
            return false;
        if (use_role && (c.role != cond.role))
            return false;
        if (use_notify && (c.notify != cond.notify))
            return false;
        if (use_type && (c.type != cond.type))
            return false;
        if (use_oside && (c.oside != cond.oside))
            return false;
        if (use_cid && (c.cid.value() != cond.cid.value()))
            return false;
        return true;
    }
    void setCondition(const _XObjectConnection &c) { cond = c; }
    /**
     * Set "use_*" to "val".
    ...
    {
        use_name = use_role = use_notify = use_type = use_oside = use_cid = val;
    }
 ...
 */
template <typename _Type> class XObject : public XMixParam
{
...
public:
     ...
     */
    typedef XObjectNotify (*FT_notify)(XObjectNotify nt, const _XObject *xo);
    typedef XObjectConnection<Type> _XObjectConnection;
    ...
    public:
        ...
        XObjectNotifier(_XObject *_starter, XObjectNotify _nt) : _XObjectScanner()
        {
    ...
    XObject(const string &objClass) :
        XMixParam(objClass), xoKey("uuid"), xoName("name"), xoType(objClass),
        xoStatus("status", ObjStatus::LOADED), xoStatus_prev("status", ObjStatus::LOADED),
    ...
    XObject() :
        XMixParam("UNNAMED_OBJ_CLASS"), xoKey("uuid"), xoName("name"), xoType("UNNAMED_OBJ_CLASS"),
        xoStatus("status", ObjStatus::LOADED), xoStatus_prev("status", ObjStatus::LOADED),
        cListVersion("xobj_clist_version", 0, -1)
    {
        addParam(&xoKey);
    ...
    string get_key() const { return xoKey.value(); }
    void set_key(const string key) { xoKey = key; }
    string get_objClass() const { return get_pname(); }
    void set_objClass(const string &objClass) { set_pname(objClass); }
    string get_name() const { return xoName.value(); }
    void set_name(const string &_name) { xoName = _name; }
    TypeLiteral get_type() const { return xoType.get_type(); }
    void set_type(TypeLiteral t) { xoType.set_type(t); }
    string get_type_string() { return xoType.value(); }
    XInt get_status() { return xoStatus.get_value(); }
    c_iterator cListEnd() { return cList.end(); }
    /**
     * Should be implemented with leaf-inherited classes.
     */
    virtual void type(Type &t) const {}
    void set_this_iter(typename XObjectList<Type>::iterator &iter) { this_iter = iter; }
    void this_iter_fini() { this_iter.fini(); }
    typename XObjectList<Type>::iterator get_this_iter() { return this_iter; }
    void inc_ruse() { ++ruse; }
    void dec_ruse()
    {
        if (ruse)
     ...
     * xml generation.
     */
    virtual string _xml(bool show_runtime, const int &indent, const string &endl) const
    {
        string sxml = "";
    ...
    string shell_xml()
    {
        ...
        if (((_XObject *)this)->chStatus(ObjStatus::PRINTING)) {
            ...
            shellXml += "<column>" + xoStatus_prev.value() + "</column>";
    ...
    void add()
    {
        if (chStatus(ObjStatus::ADDING)) {
            ...
            throw Exception("Can't prepare object : " + get_name() + " for adding!",
                            TracePoint("xobject"));
        }
    }
    /**
    ...
    void del()
    {
        if (chStatus(ObjStatus::DELETING)) {
            if (is_busy()) {
                bkStatus();
                throw Exception("Can't delete busy object !", TracePoint("xobject"));
            }
            try {
                _del();
            } catch (Exception &e) {
            ...
            throw Exception("Can't prepare object : " + get_name() + " for deleting!",
                            TracePoint("xobject"));
        }
    }
    /**
     ...
     */
    bool mod_prepare() { return chStatus(ObjStatus::MODIFYING); }
    /**
     ...
     */
    bool reload_prepare() { return chStatus(ObjStatus::RELOADING); }
    /**
    ...
    ScannerAction scan(_XObjectScanner &s, const bool async = false)
    {
        ...
        case _XObjectScanner::SA_FORWARD:
             ...
             */
            for (c_iterator iter = cList.begin(); iter != cList.end(); ++iter) {
                if (!s.match(*iter))
                    continue;
    ...
    string xml_connectionsList() const
    {
        ...
        for (c_const_iterator iter = cList.begin(); iter != cList.end(); ++iter) {
            ret += iter->xml();
        }
        ret += "</xobj_clist>";
        return ret;
     ...
     */
    virtual bool operator==(const _XObject &lvalue) const { return false; }
    /**
    ...
    {
        for (c_iterator iter = cList.begin(); iter != cList.end(); ++iter)
            if (s.match(*iter) && ((*(iter->oside))->get_key() == objID))
                return iter;
        return cList.end();
    }
    /**
    ...
    {
        for (c_iterator iter = citer; iter != cList.end(); ++iter)
            if (s.match(*iter))
                return iter;
        return cList.end();
    }
...
protected:
    ...
    bool chStatus(XInt _status)
    {
        ...
        while (1) {
            switch (ObjStatus::canTransit(xoStatus.get_value(), _status)) {
     ...
     */
    virtual bool mayConnect(_XObjectConnection &c) { return true; }
    /**
    ...
    c_iterator xdelConnection(c_iterator &iter)
    {
        ...
        std::thread([this, iter, objIter]() { cList.xerase(iter); }).detach();
        return ret;
    }
    /**
     ...
     */
    virtual XObjectNotify notify(XObjectNotify nt, const _XObject *xo) { return OBJ_NOTIFY_NONE; }
    /**
    ...
    virtual void _add()
    {
        throw Exception("Unsupported function for this object !", TracePoint("xobject"));
    }
    virtual void _del()
    {
        throw Exception("Unsupported function for this object !", TracePoint("xobject"));
...
template <class Type> class XObjectRepository;
...
public:
    ...
    XObjectList(const string &name, const string &logName) : list(name), logs(logName)
    {
        list.enable_smap();
    ...
    string get_name() const { return list.get_pname(); }
    /**
    ...
    iterator loadObj(_XObject *xo, bool clone = true)
    {
        ...
        try {
            ...
            pthread_mutex_unlock(&dup_lock);
            e.addTracePoint(TracePoint("xobject"));
            string err = "Can't create new object from " + xo->get_name();
    ...
    iterator add(_XObject *xo, bool clone = true)
    {
        ...
        try {
            ...
            e.addTracePoint(TracePoint("xobject"));
            if (e.is_failed()) {
                _XObject *nxo = static_cast<_XObject *>(*iter);
    ...
    iterator add(const string &key)
    {
        ...
        if (iter == end()) {
            unlock();
            throw Exception("Object doesn't exist !", TracePoint("xobject"));
        }
        unlock();
        try {
            _add(iter);
    ...
    iterator del(iterator &iter)
    {
        ...
        try {
            ...
            e.addTracePoint(TracePoint("xobject"));
            if (e.is_failed()) {
                string err = obj->get_name() + " couldn't be deleted: " + e.what();
                logs << LogLevel::ERROR << err;
                throw e;
            } else if (e.is_nok()) {
                string err = obj->get_name() + " was deleted but some errors\
					occured: " +
                             e.what();
    ...
    bool delAll()
    {
        ...
        for (int i = 0; i < size; ++i)
            isOK = delType(priority.at(size - (1 + i))) && isOK;
    ...
    iterator mod(_XObject *xo, bool reload = false)
    {
        ...
        try {
            ...
            e.addTracePoint(TracePoint("xobject"));
            if (e.is_failed()) {
                string err = old_obj->get_name() + " couldn't be modified: " + e.what();
                logs << LogLevel::ERROR << err;
                throw e;
            } else if (e.is_nok()) {
                string err = old_obj->get_name() + " was modified but some errors\
					occured: " +
                             e.what();
                logs << LogLevel::INFO << err;
                throw e;
     ...
     */
    template <typename Query> iterator query(Query _query)
    {
     ...
     */
    template <typename Query> iterator query_lock(Query _query)
    {
        rdlock();
     ...
     */
    template <class T, class CallBack> void iterate(CallBack callback)
    {
        for (auto iter = begin(); iter != end(); ++iter) {
            callback(((T *)*iter));
        }
     ...
     */
    template <class T, class CallBack> bool iterate_until(CallBack callback)
    {
        for (auto iter = begin(); iter != end(); ++iter) {
            if (!callback(((T *)*iter)))
                return false;
     ...
     * Use when you are sure about lock on list.
     */
    iterator query(const string &objkey) { return list.find(objkey); }
    /**
     * Lock the list and query specified object.
     */
    ...
    iterator query_name(const std::string name)
    {
        ...
        for (objectIterator = list.begin(); objectIterator != list.end(); ++objectIterator) {
     ...
     * \return true: objects loaded, false: loading canceled.
     */
    bool loadXmlDoc(string xdoc, XParam::XmlParser *parser = NULL, bool checksum = true,
                    const string &iv = "")
    {
        if (repo->cancelLoading())
            return false;
        ...
        set_xmlDoc(xdoc);
        try {
            list.loadXmlDoc(xdoc, parser, checksum, (iv == "") ? xdoc : iv);
        } catch (Exception &e) {
            unlock();
            e.addTracePoint(TracePoint("xobject"));
            throw e;
    ...
    bool addLoadedObjects()
    {
        ...
        if (size) {
            ...
            for (int i = 0; i < size; ++i) {
                if (repo->cancelLoading())
                    return false;
                isOK = addLoadedObjectsType(priority.at(i)) && isOK;
            }
        } else {
    ...
    void saveXmlDoc(const string &xdoc, bool show_runtime = false, const int &indent = 0,
                    bool with_endl = false, bool checksum = true, const string &iv = "")
    {
        try {
            list.saveXmlDoc(xdoc, show_runtime, indent, with_endl, checksum,
                            (iv == "") ? xdoc : iv);
        } catch (Exception &e) {
            unlock();
            e.addTracePoint(TracePoint("xobject"));
    ...
    string xml(bool show_runtime = false, const int &indent = 0, bool with_endl = false)
    {
    ...
    string shell_xml()
    {
        ...
        for (listIterator = begin(); listIterator != end(); listIterator++) {
    ...
    void set_xmlDoc(string xdoc) { xmlDoc = xdoc; }
    string get_xmlDoc() { return xmlDoc; }
    bool has_xmlDoc() { return !get_xmlDoc().empty(); }
    void set_priority(const Priority &p) { priority = p; }
    Priority get_priority() { return priority; }
    /**
     * By this we can define type periorities by pushing
     * them from first to last.
     */
    void pushPriority(TypeLiteral type) { priority.push_back(type); }
    void wrlock() { pthread_rwlock_wrlock(&list_lock); }
    void rdlock() { pthread_rwlock_rdlock(&list_lock); }
    void unlock() { pthread_rwlock_unlock(&list_lock); }
    iterator begin() { return list.begin(); }
    iterator end() { return list.end(); }
    const_iterator begin() const { return list.begin(); }
    const_iterator end() const { return list.end(); }
    const_iterator const_begin() const { return list.const_begin(); }
    const_iterator const_end() const { return list.const_end(); }
    bool load()
    {
        try {
            if (has_xmlDoc())
            ...
            string err = "Can't load " + get_name() + "list !:" + e.what();
            logs << LogLevel::ERROR << err;
            return false;
        }
        return true;
    }
    bool unload() { return delAll(); }
    void set_repo(XObjectRepository<Type> *_repo) { repo = _repo; }
...
private:
     ...
     */
    void verifyObjectDuplication(_XObject *new_obj, _XObject *old_obj = NULL)
    {
        for (iterator iter = list.begin(); iter != list.end(); ++iter) {
            _XObject *xobj = (_XObject *)*iter;
            if (xobj == old_obj)
            ...
            if (xobj->chStatus(ObjStatus::QUERYING)) {
                if ((*xobj == *new_obj) || (xobj->get_key() == new_obj->get_key())) {
                    xobj->bkStatus();
    ...
    void _add(iterator &iter)
    {
        ...
        try {
            ...
            e.addTracePoint(TracePoint("xobject"));
            if (e.is_failed()) {
                string err = nobj->get_name() + " couldn't be added: " + e.what();
                logs << LogLevel::INFO << err;
                throw e;
            } else if (e.is_nok()) {
                nobj->set_this_iter(iter);
                string err = nobj->get_name() + " was added with error: " + e.what();
                logs << LogLevel::INFO << err;
                throw e;
            }
        }
 ...
 * search or access.
 */
template <typename Type> class XObjectRepository
{
public:
    typedef XObjectList<Type> _XObjectList;
    typedef XObject<Type> _XObject;
     ...
     */
    XObjectRepository(string _name, string logName) : name(_name), logs(logName)
    {
     ...
     * biggest to smallest.
     */
    _XObjectList *regObjList(ListID listID, const string &name, _XObjectList *_xolist = NULL)
    {
        wrlock();
        if (repo.find(listID) != repo.end()) {
            unlock();
            throw Exception("Duplicate list !", TracePoint("xobject"));
        }
        _XObjectList *xolist =
            (_xolist) ? _xolist : new _XObjectList(name, logs.get_subSystemName());
        if (xolist) {
            repo[listID] = xolist;
            xolist->set_repo(this);
        } else {
            unlock();
            throw Exception("Can't allocate new list !", TracePoint("xobject"));
        }
        unlock();
        return xolist;
    }
    ...
    bool loadXmlStr(ListID listID, const string &xstr, XParam::XmlParser *parser = NULL)
    {
        bool ret;
        rdlock();
        try {
    ...
    bool loadXmlDoc(ListID listID, const string &xdoc, XParam::XmlParser *parser = NULL)
    {
        bool ret;
        rdlock();
        try {
    ...
    bool addLoadedObjects()
    {
        bool isOK = true;
        rdlock();
        for (list_iterator iter = repo.begin(); iter != repo.end(); ++iter)
            isOK = iter->second->addLoadedObjects() && isOK;
        unlock();
        return isOK;
    }
    void saveXmlDoc(ListID listID, const string &xdoc, bool show_runtime = false,
                    const int &indent = 0, bool with_endl = false)
    {
        rdlock();
        try {
            list_iterator iter = findList(listID);
            iter->second->saveXmlDoc(xdoc, show_runtime, indent, with_endl);
        } catch (Exception &e) {
            unlock();
            e.addTracePoint(TracePoint("xobject"));
            throw e;
    ...
    string xml(ListID listID, bool show_runtime = false, const int &indent = 0,
               bool with_endl = false)
    {
        ...
        try {
            list_iterator iter = findList(listID);
            ret = iter->second->xml(show_runtime, indent, with_endl);
        } catch (Exception &e) {
            unlock();
            e.addTracePoint(TracePoint("xobject"));
            throw e;
    ...
    string xml(bool show_runtime = false, const int &indent = 0, bool with_endl = false)
    {
        ...
        try {
            for (list_iterator iter = repo.begin(); iter != repo.end(); ++iter)
                str += iter->second->xml(show_runtime, indent, with_endl);
        } catch (Exception &e) {
            unlock();
            e.addTracePoint(TracePoint("xobject"));
            throw e;
    ...
    void save()
    {
        ...
        for (list_iterator iter = repo.begin(); iter != repo.end(); ++iter) {
            try {
                ...
                string error = "can't save " + iter->second->get_name() + ": " + e.what();
    ...
    bool unload()
    {
         ...
         */
        for (list_riterator riter = repo.rbegin(); riter != repo.rend(); ++riter)
     ...
     * lambda must return true or false.
     */
    template <typename Query> iterator query(ListID listID, Query _query)
    {
        try {
            list_iterator iter = findList(listID);
     ...
     * lambda must return true or false.
     */
    template <typename Query> iterator query_lock(ListID listID, Query _query)
    {
        iterator ret;
        rdlock();
        try {
     ...
     */
    template <typename Query> iterator query_lock_locklst(ListID listID, Query _query)
    {
        iterator ret;
        rdlock();
        try {
     ...
     */
    template <typename Query> iterator query_lock_locklst(Query _query)
    {
        iterator objiter;
        rdlock();
        for (list_iterator iter = repo.begin(); iter != repo.end(); ++iter) {
     ...
     */
    iterator query_name_locklist(const ListID listID, const std::string name)
    {
        list_iterator listIterator;
     ...
     * iterator.is_end() returns true
     */
    iterator query_name_lock_locklist(const ListID listID, const std::string name)
    {
        iterator objectIterator;
     ...
     */
    bool query_name_lock_locklist(const std::string name, iterator &objectIterator)
    {
        list_iterator listIterator;

        rdlock();
        for (listIterator = repo.begin(); listIterator != repo.end(); ++listIterator) {
            objectIterator = listIterator->second->query_name_lock(name);
            if (!objectIterator.is_end()) {
                unlock();
     ...
     */
    void query_type_locklst(ListID listID, TypeLiteral _type, std::vector<iterator> &out)
    {
        try {
            list_iterator iter = findList(listID);
     ...
     * \param [out] out list of found objects of type "_type".
     */
    void query_type_lock_locklst(ListID listID, TypeLiteral _type, std::vector<iterator> &out)
    {
        rdlock();
        try {
            list_iterator iter = findList(listID);
    ...
    void unlock() { pthread_rwlock_unlock(&rw_lock); }
    void rdlock() { pthread_rwlock_rdlock(&rw_lock); }
    void wrlock() { pthread_rwlock_wrlock(&rw_lock); }
    ...
    void set_cancelLoading(FT_cancelLoading n_cancelLoading) { _cancelLoading = n_cancelLoading; }
...
protected:
    ...
    list_iterator findList(ListID listID)
    {
        ...
        if (iter == repo.end()) {
            throw Exception("Object List doesn't exist !", TracePoint("xobject"));
        }
        return iter;
    }
