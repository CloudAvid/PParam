...
void *list_update(void *arguments){
	XList<T>* list = (XList<T>*) arguments;
        ...
        while (true) {
                iterator = list->begin();
	        for (int i = 0; i < rand()%(static_cast<int>(list->size()) + 1); i++) {
	        	iterator.operator++();
		}
		*iterator = rand();
...
void *list_remove(void *arguments) {
	...
	while (true) {
		...
		for (int i = 0; i < rand()%(static_cast<int>(liststruct->list->size()) + 1); i++) {
			iterator.operator++();
		}
		...
		timee = time(nullptr);
		if ((timee - times) > 1) {
			cout << "the erase function is bigger than 1s and is: " << timee - times 
				<< " seconds" <<endl;
		}
	...
	struct signal_struct<T>* signalstruct = (signal_struct<T>*) argument;
		...
		if (s != 0) {
			...
			switch (sig) {
				...
				case SIGALRM:
					cout << "the list is ";
					for (XListIterator<int> iterator = signalstruct->list->begin(); iterator != signalstruct->list->end(); ++iterator) {
						cout << "<-->" << *iterator;
					}
					cout << "<-->" << endl;
					break;
...
int main(int argc, char ** argv) {
	...
	if (argc == 5) {
		cout << "number of reader thread(s):" << argv[1] << " writer thread(s):" << 
			argv[2] << " updater thread(s):" << argv[3] << " remover thread(s):"
		       	<< argv[4] << endl;
		...
		cout << "number of reader thread(s):" << READER_THREADS << " writer thread(s):"
			<< WRITER_THREADS << " updater thread(s):" << UPDATER_THREADS <<
	...
	int rc = 0;
	for (vector<pthread_t>::iterator it = reader.begin(); it != reader.end(); ++it) {
		cout << "list_read(): creating thread, " << endl;
                rc = pthread_create(&*it, NULL, list_read<int>, (void *) &xlist);
                if (rc) {
	...
	for (vector<pthread_t>::iterator it = writer.begin(); it != writer.end(); ++it) {
        	cout << "list_write(): creating thread, " << endl;
		rc = pthread_create(&*it, NULL, list_write<int>, (void *) &list);
                if (rc) {
	...
	for (vector<pthread_t>::iterator it = updater.begin(); it != updater.end(); ++it) {
        	cout << "list_update(): creating thread, " << endl;
                rc = pthread_create(&*it, NULL, list_update<int>, (void *) &xlist);
		if(rc){
			cout<< "Error:unable to create read thread, " << endl;
			exit(-1);
		}
	...
	for (vector<pthread_t>::iterator it = remover.begin(); it != remover.end(); ++it) {
        	cout << "list_remove(): creating thread, " << endl;
		rc = pthread_create(&*it, NULL, list_remove<int>, (void *) &list);
                if (rc) {
                	cout<< "Error:unable to create read thread, " << endl;
			exit(-1);
		}
