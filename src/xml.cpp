...
Tlist get_children_common(const std::string &name, xmlNode *child)
{
	Tlist children;
        while (child) {
...
Node::Node(xmlNode *_node) :
		node(_node)
{
	if (!node)
		throw Exception("xmlNode pointer cannot be nullptr.",
						TracePoint("xml"));
	node->_private = this;
}

xmlNode* Node::get_node() const
{
	return node;
}
...
std::string Element::get_attribute(const std::string &key) const
{
	const xmlChar* const value =
		xmlGetProp(get_node(),(const xmlChar*)key.c_str());
...
ContentNode::ContentNode(xmlNode* _node) :
					Node(_node)
{
}
...
std::string ContentNode::get_content() const
{
	return get_node()->content ?
		std::string((const char*)get_node()->content) :
		std::string("");
}
...
CommentNode::CommentNode(xmlNode* _node) :
					ContentNode(_node)
{
}
...
CDataNode::CDataNode(xmlNode* _node) :
				ContentNode(_node)
{
}
...
TextNode::TextNode(xmlNode* _node) :
					ContentNode(_node)
{
}
...
void Document::parseXmlStr(const std::string &xmlStr)
{
	...
	document = xmlParseDoc((const xmlChar*) xmlStr.c_str());
		...
		if (error)
			throw Exception(("Can't parse document : " +
				std::string(error->message)), TracePoint("xml"));
		else
			throw Exception("Can't parse document : xml is empty.",
							TracePoint("xml"));
        }
...
std::string Document::queryXml(const std::string &query,
			const std::string &partialTag)
{
	...
	if (context == NULL) {
		...
		throw Exception(("Can't execute query : " +
			errorStr), TracePoint("xml"));
	}

	result = xmlXPathEvalExpression(
		((const xmlChar*) query.c_str()), context);
	...
	for (int i = 0; i < result->nodesetval->nodeNr; ++i) {
		if (xmlNodeDump(buffer, document,
				result->nodesetval->nodeTab[i], 0, 0) == -1) {
...
xmlDoc* Document::get_document() const
{
	return document;
}
...
void Parser::parse_memory(const std::string &xmlStr)
{
	document->parseXmlStr(xmlStr);
}

void Parser::parse_file(const std::string &filePath)
{
	document->parseXmlFile(filePath);
}
