...
const string LogLevel::typeString[LogLevel::MAX] = {"emergency", "alert",  "critical", "error",
                                                    "warning",   "notice", "info",     "debug"};

LogSystem::LogSystem(const string &_subSystemName) { set_subSystemName(_subSystemName); }
...
void LogSystem::setLogFacility(int _logFacility) { logFacility = _logFacility; }

void LogSystem::setLogMask(int _mask) { mask = _mask; }
...
void LogSystem::_sendLogMsg(const char *logMsg)
{
    if (mask >= subSystemLogLevel) {
        syslog(logFacility | subSystemLogLevel, "%s: %s", subSystemName_c_str, logMsg);
    }
}
